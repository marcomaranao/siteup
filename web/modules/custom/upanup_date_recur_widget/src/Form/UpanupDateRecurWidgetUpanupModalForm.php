...
namespace Drupal\upanup_date_recur_widget\Form;
...
use Drupal\upanup_date_recur_widget\UpanupDateRecurWidgetUtilityTrait;
use Drupal\upanup_date_recur_widget\UpanupDateRecurWidgetWidgetFieldsTrait;
use Drupal\upanup_date_recur_widget\UpanupDateRecurWidgetWidgetOptions;
use Drupal\upanup_date_recur_widget\Plugin\Field\FieldWidget\UpanupDateRecurWidgetUpanupWidget;
use Symfony\Component\DependencyInjection\ContainerInterface;

/**
 * Generate a form designed for display in modal.
 */
class UpanupDateRecurWidgetUpanupModalForm extends FormBase {

  use UpanupDateRecurWidgetWidgetFieldsTrait;
  use UpanupDateRecurWidgetUtilityTrait;
  ...
  /**
   * Constructs a new UpanupDateRecurWidgetUpanupModalForm.
   *
  ...
  public function getFormId() {
    return 'upanup_date_recur_widget_upanup_modal';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    $form['#attached']['library'][] = 'upanup_date_recur_widget/upanup_date_recur_widget_upanup_widget_modal_form';
    $form['#attached']['library'][] = 'core/drupal.ajax';
    $form['#theme'] = 'upanup_date_recur_widget_upanup_widget_modal_form';

    $collection = $this->tempStoreFactory
      ->get(UpanupDateRecurWidgetUpanupWidget::COLLECTION_MODAL_STATE);

    $rrule = $collection->get(UpanupDateRecurWidgetUpanupWidget::COLLECTION_MODAL_STATE_KEY);
    $dtStartString = $collection->get(UpanupDateRecurWidgetUpanupWidget::COLLECTION_MODAL_STATE_DTSTART);

    if (!empty($dtStartString)) {
      $dtStart = \DateTime::createFromFormat(UpanupDateRecurWidgetUpanupWidget::COLLECTION_MODAL_STATE_DTSTART_FORMAT, $dtStartString);
    }
    else {
    ...
    $form['ends']['#theme'] = 'upanup_date_recur_widget_upanup_widget_modal_form_ends';

    $endsModeDefault =
      $endsDate ? UpanupDateRecurWidgetWidgetOptions::ENDS_MODE_ON_DATE :
      ($count > 0 ? UpanupDateRecurWidgetWidgetOptions::ENDS_MODE_OCCURRENCES : UpanupDateRecurWidgetWidgetOptions::ENDS_MODE_INFINITE);
    $form['ends']['ends_mode'] = [
      '#type' => 'radios',
      '#title' => $this->t('Ends'),
      '#options' => [
        UpanupDateRecurWidgetWidgetOptions::ENDS_MODE_INFINITE => $this->t('Never'),
        UpanupDateRecurWidgetWidgetOptions::ENDS_MODE_ON_DATE => $this->t('On'),
        UpanupDateRecurWidgetWidgetOptions::ENDS_MODE_OCCURRENCES => $this->t('After'),
      ],
    ...
    $form['ends']['ends_count']['#states']['enabled'][] = [
      ...
      ':input[name="ends_mode"]' => ['value' => UpanupDateRecurWidgetWidgetOptions::ENDS_MODE_OCCURRENCES],
    ];
    ...
    $form['ends']['ends_date']['#states']['enabled'][] = [
      // This applies correctly but Drupal has no theming for disabled dates.
      ':input[name="ends_mode"]' => ['value' => UpanupDateRecurWidgetWidgetOptions::ENDS_MODE_ON_DATE],
    ];
    ...
    $form['actions']['submit'] = [
      ...
      '#ajax' => [
        ...
        'url' => Url::fromRoute('upanup_date_recur_widget_widget.upanup_modal_form'),
        'options' => [
          'query' => [
  ...
  public function ajaxSubmitForm(array &$form, FormStateInterface $form_state) {
    ...
    // Ends mode.
    if ($endsMode === UpanupDateRecurWidgetWidgetOptions::ENDS_MODE_OCCURRENCES) {
      $parts['COUNT'] = (int) $form_state->getValue('ends_count');
    }
    elseif ($endsMode === UpanupDateRecurWidgetWidgetOptions::ENDS_MODE_ON_DATE && $endsDate instanceof DrupalDateTime) {
    ...
    $collection = $this->tempStoreFactory
      ->get(UpanupDateRecurWidgetUpanupWidget::COLLECTION_MODAL_STATE);
    $collection->set(UpanupDateRecurWidgetUpanupWidget::COLLECTION_MODAL_STATE_KEY, $ruleString);

    $refreshBtnName = sprintf('[name="%s"]', $collection->get(UpanupDateRecurWidgetUpanupWidget::COLLECTION_MODAL_STATE_REFRESH_BUTTON));
    $response
